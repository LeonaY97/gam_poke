...
class GameController {
    constructor(room, broadcastFn, privateFn) {
        this.turnTimer = null;
        this.showdownTimer = null;
        this.disconnectTimers = new Map();
        this.botPlayers = new Set();
        /** 等待借入决策的真人玩家 playerId 集合 */
        this.pendingBorrowers = new Set();
        /** 等待确认关闭结算画面的真人玩家 playerId 集合 */
        this.pendingHandResultAcks = new Set();
        this.room = room;
    ...
    dealHoleCards() {
        ...
        for (const gp of this.game.players) {
            const { cards: holeCards, remainingDeck: newDeck } = (0, deck_1.dealCards)(remaining, 2);
            remaining = newDeck;
            gp.hand = holeCards.map(deck_1.cardIndexToCard);
        }
    ...
    sendTurnToPlayer(gp) {
        ...
        const options = (0, deck_1.getAvailableActions)(player.chips, gp.totalBet, highestBet, this.room.settings.bigBlind, isFirstToAct, lastRaiseAmount);
    ...
    evaluateBotHandStrength(gp) {
        ...
        const result = (0, deck_1.getBestHand)(gp.hand, communityCards);
        const strengthMap = {
    ...
    handleAction(playerId, action, amount) {
        ...
        switch (action) {
            ...
            case 'raise': {
                if (amount < toCall)
                    return { success: false, error: '加注金额不能低于跟注金额' };
    ...
    hasPlayerActedInRound(playerId) {
        return this.game.betHistory.some(b => b.phase === this.game.phase && b.playerId === playerId && b.action !== 'fold');
    }
    hasActionInRound() {
        return this.game.betHistory.some(b => b.phase === this.game.phase && b.action !== 'fold');
    }
    endBettingRound() {
        const nextPhase = (0, deck_1.getNextPhase)(this.game.phase);
        ...
        const { cards: newCards, remainingDeck } = (0, deck_1.dealCards)(this.game.deck, neededCards);
        this.game.deck = remainingDeck;
        this.game.communityCards.push(...newCards.map(deck_1.cardIndexToCard));
    ...
    doShowdown() {
        ...
        const results = (0, deck_1.compareHands)(activePlayers.map(gp => ({
            playerId: gp.playerId,
            hand: gp.hand,
            communityCards: this.game.communityCards,
        })));
        // 按手牌强度从强到弱排序
        const sortedResults = [...results].sort((a, b) => this.getHandNumericValue(b.bestCards) - this.getHandNumericValue(a.bestCards));
        // 按 side pot 分层分配奖金
        ...
        for (const sp of sidePots) {
            ...
            const layerWinners = eligibleResults.filter(r => this.getHandNumericValue(r.bestCards) === topVal);
        ...
        const handResult = {
            ...
            allHands: activePlayers.map(gp => {
                const best = (0, deck_1.getBestHand)(gp.hand, this.game.communityCards);
                const player = this.getPlayer(gp.playerId);
                return {
                    playerId: gp.playerId,
                    nickname: player?.nickname || 'Unknown',
    ...
    endGameWithSingleWinner(gp) {
        ...
        const handResult = {
            ...
            allHands: this.game.players.map(gp2 => {
                const best = (0, deck_1.getBestHand)(gp2.hand, this.game.communityCards);
     ...
     * 设 60s 兜底超时防止卡死。
     */
    waitForHandResultAcks() {
        const realPlayers = this.getPlayersArray().filter(p => !this.botPlayers.has(p.id) && p.isConnected);
     ...
     * 当所有待借入玩家都决策完，触发下一局。
     */
    resolveBorrower(playerId, borrow) {
    ...
    handlePlayerDisconnect(playerId) {
        if (this.botPlayers.has(playerId))
            return;
    ...
    updatePot() {
        this.game.pot = this.game.players.reduce((sum, p) => sum + p.totalBet, 0);
    ...
    getHandNumericValue(cards) {
        // 直接使用 deck.ts 的 getHandValue，它正确地按频率排序（对子/三条优先于 kicker）
        return (0, deck_1.getHandValue)(cards);
    }
    getPlayer(playerId) {
        if (this.room.players instanceof Map) {
            return this.room.players.get(playerId) || null;
        }
        return this.room.players[playerId] || null;
    }
    getPlayersArray() {
        if (this.room.players instanceof Map) {
     ...
     * 返回是否轮到该玩家行动，便于上层决定是否重发 your_turn。
     */
    resendStateForPlayer(playerId) {
        if (this.game.phase === 'waiting' || this.game.phase === 'showdown')
            return;
        // 1. 重发手牌
        const gp = this.game.players.find(p => p.playerId === playerId);
        if (!gp || this.botPlayers.has(playerId))
            return;
        if (gp.hand.length > 0) {
